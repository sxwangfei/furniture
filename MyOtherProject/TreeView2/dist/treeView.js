...
var configFun = function configFun() {
    var t = this;
};
configFun.prototype = {
    data: {
    ...
    buildTree: function buildTree(option) {
        var t = this;
        var type = 0;
        ...
        var levelBgColorArray = ["#96cbff", "#ceffcf", "#dedeff"];
        if (typeof option.levelBgColorArray != 'undefined') {
            levelBgColorArray = option.levelBgColorArray;
        }
        option.levelBgColorArray = levelBgColorArray;
        if (typeof option.check != 'undefined' && option.check.length > 0) {
            for (var i = 0; i < data.length; i++) {
                var id = data[i].id;
                data[i]['check'] = false;
                for (var k = 0; k < option.check.length; k++) {
                    if (id == option.check[k]) {
                        data[i]['check'] = true;
                        break;
                    };
                };
        ...
        for (var i = 0; i < data.length; i++) {
            if (data[i].pid == 0) {
                ...
                ///没有选择框
                if (option.type == 0) {
                    if (levelRef.hasOwnProperty(data[i].id) == false) {
                        levelRef[data[i].id] = 0;
                    }
                    listHtml = $('<div class="f-treeList f-treeListOne" style="background-color:' + t.getLevelColor(option, levelRef[data[i].id]) + ';">' + '<div class="f-treeList-top">' + '<div class="f-treeList-old"><span class="f-iconJia"></span></div>' + '<div class="f-treeList-title">' + '<div class="f-treeList-titleImg"><span class="f-iconBi"></span></div>' + '<div class="f-treeList-titleP"><div class="f-top">' + data[i].title + '</div>' + '<div class="f-center"><span class="f-startdate">计划开始：' + data[i].start + '</span><span class="f-finishdate">计划结束：' + data[i].finish + '</span><span class="f-duration">工期：' + data[i].duration + '</span></div>' + '<div class="f-bottom"><span>进度：</span><div class="f-progress"><span class="f-progressValue" style="width:' + parseInt(data[i].progress * 0.5) + 'px;"></span></div>' + '<span class="f-progressText">' + data[i].progress + '%</span>' + '<span class="f-progressStatus">延期完成</span>' + '</div></div>' + '<a class="f-link" ></a>' + '</div></div></div>');
                };
                ///有选择框
                if (option.type == 1) {
                    ...
                    listHtml = $('<div class="f-treeList f-treeListOne">' + '<div class="f-treeList-top">' + '<div class="f-treeList-old"><span class="f-iconJia"></span></div>' + '<div class="f-treeList-title">' + '<div class="f-treeList-radio">' + radioIconHtml + '<input value="' + data[i].id + '"/>' + '</div>' + '<div class="f-treeList-titleImg"><span class="f-iconBi"></span></div>' + '<p class="f-treeList-titleP">' + data[i].title + '</p>' + '</div></div></div>');
                };
        ...
        for (var i = 0; i < one.length; i++) {
            ...
            t.treeFor({
                ...
                dataHover: one[i],
                levelRef: levelRef,
                levelBgColorArray: levelBgColorArray
            });
        };
        console.log(levelRef);
    },
    treeFor: function treeFor(option) {
        var t = this;
        var data = option.data;
        var dataHover = option.dataHover;
        var fatherHtml = $('<div class="f-treeListWrapper hide"><div class="f-treeList-lineShu"></div></div>');
        var listHtml = '';
        var self = []; //子集
        var next = [];
        var levelRef = option.levelRef;
        var levelBgColorArray = option.levelBgColorArray;
        for (var i = 0; i < data.length; i++) {
            if (data[i].pid == dataHover.id) {
                if (option.type == 0) {
                    if (levelRef.hasOwnProperty(dataHover.id)) {
                        var leint = parseInt(levelRef[dataHover.id]) + 1;
                        levelRef[data[i].id] = leint;
                    }
                    listHtml = $('<div class="f-treeList f-treeListOne" style="background-color:' + t.getLevelColor(option, levelRef[data[i].id]) + ';">' + '<div class="f-treeList-top">' + '<div class="f-treeList-old"><span class="f-iconJia"></span></div>' + '<div class="f-treeList-title">' + '<div class="f-treeList-titleImg"><span class="f-iconBi"></span></div>' + '<div class="f-treeList-titleP"><div class="f-top">' + data[i].title + '</div>' + '<div class="f-center"><span class="f-startdate">计划开始：' + data[i].start + '</span><span class="f-finishdate">计划结束：' + data[i].finish + '</span><span class="f-duration">工期：' + data[i].duration + '</span></div>' + '<div class="f-bottom"><span>进度：</span><div class="f-progress"><span class="f-progressValue" style="width:' + parseInt(data[i].progress * 0.5) + 'px;"></span></div>' + '<span class="f-progressText">' + data[i].progress + '%</span>' + '<span class="f-progressStatus">延期完成</span>' + '</div></div>' + '<a class="f-link" ></a>' + '</div></div></div>');
                };
                if (option.type == 1) {
                    ...
                    listHtml = $('<div class="f-treeList">' + '<div class="f-treeList-top">' + '<div class="f-treeList-old"><span class="f-iconJia"></span></div>' + '<div class="f-treeList-title">' + '<div class="f-treeList-radio">' + radioIconHtml + '<input value="' + data[i].id + '"/>' + '</div>' + '<div class="f-treeList-titleImg"><span class="f-iconBi"></span></div>' + '<p class="f-treeList-titleP">' + data[i].title + '</p>' + '</div></div></div>');
                };
                fatherHtml.append(listHtml);
        ...
        if (self.length == 0 && option.dataHover.pid != 0) {
            var selfDom = option.dataHover.list;
    ...
    treeJson: function treeJson(option) {
        var wrapper = $(option.wrapper);
        ...
        option.success(radioArr);
    },
    getLevelColor: function getLevelColor(option, level) {
        var t = this;
        if (option.levelBgColorArray.length > 0 && option.levelBgColorArray.length >= level) {
            return option.levelBgColorArray[level];
        } else {
            return "";
        }
    }
};
var treeView = new configFun();
